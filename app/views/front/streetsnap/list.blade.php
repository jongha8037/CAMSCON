...
var ListView={
	...
	appendSnaps:function(snaps) {
		...
		var slen=snaps.length;for(var i=0;i<slen;i++) {
			...
			$('<span class="pointer likes-display icon-right-open-big"></span>').appendTo(snap);
			$('<span class="total-likes likes-display" data-target-type="StreetSnap" data-target-id=""></span>').attr('data-target-id', snaps[i].id).text(snaps[i].cached_total_likes).appendTo(snap);
